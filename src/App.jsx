...
const getDeadlineBadgeClass = (status, darkMode) => {
  if (status === 'retard') {
    return darkMode ? 'bg-red-950/60 text-red-300' : 'bg-red-100 text-red-700';
  }
  if (status === 'urgent') {
    return darkMode ? 'bg-amber-950/60 text-amber-300' : 'bg-amber-100 text-amber-700';
  }
  return darkMode ? 'bg-indigo-950/60 text-indigo-300' : 'bg-indigo-100 text-indigo-700';
};
...
const TaskSubtasks = ({ task, darkMode, onToggle, onAddSubtask, archived = false }) => {
  ...
  return (
    <div className="mb-4">
      ...
      {subtasks.length > 0 && (
        <>
          <div className={`h-1.5 rounded-full overflow-hidden mb-3 ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
          ...
          <div className="space-y-2 mb-3">
            {subtasks.map((subtask) => (
              <label
                key={subtask.id}
                className={`flex items-center gap-2 text-xs rounded-xl px-3 py-2 border ${darkMode ? 'border-slate-800 bg-slate-950/60' : 'border-slate-200 bg-slate-50'}`}
              >
                <input
                  type="checkbox"
                ...
                />
                <span className={subtask.done ? 'line-through text-slate-400' : darkMode ? 'text-slate-200' : 'text-slate-700'}>
                  {subtask.text}
                </span>
              </label>
            ))}
      ...
      {!archived && (
        <div className="flex gap-2 mt-2">
          <input
            ...
            className={`flex-1 px-3 py-2 rounded-xl outline-none text-xs ${darkMode ? 'bg-slate-800 text-white placeholder:text-slate-500' : 'bg-slate-50 text-slate-700 placeholder:text-slate-400'}`}
          />
          <button
            type="button"
...
const App = () => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        if (data?.payload) {
          ...
          setDarkMode(payload.darkMode ?? false);
          setActiveView(payload.activeView ?? 'board');
          setCurrentLogo(payload.currentLogo ?? 'rocket');
          setTags(payload.tags ?? DEFAULT_TAGS);
          setColumnNames(payload.columnNames ?? DEFAULT_COLUMN_NAMES);
          setTasks(payload.tasks ?? DEFAULT_TASKS);
          setArchivedTasks(payload.archivedTasks ?? []);
          setTimesheetEntries(payload.timesheetEntries ?? DEFAULT_TIMESHEET_ENTRIES);
        }
  ...
  useEffect(() => {
    if (!db || !user || !dataHydrated) return undefined;
  ...
  const signInWithGoogle = async () => {
    ...
    try {
      ...
      setSaveError('Connexion Google impossible. Vérifie la configuration Firebase.');
    }
  };
  ...
  const addTag = () => {
    ...
    const newTag = { id: Date.now().toString(), name: trimmedName, color: randomColor };
  ...
  const removeTag = (id, name) => {
    ...
    setArchivedTasks((prev) => prev.map((task) => (task.tag === name ? { ...task, tag: 'Général' } : task)));

    if (form.tag === name) setForm((prev) => ({ ...prev, tag: 'Général' }));
    if (timesheetForm.project === name) setTimesheetForm((prev) => ({ ...prev, project: 'Général' }));
  ...
  const stats = {
    ...
    retard: tasks.filter((task) => getTaskStatus(task.dueDate) === 'retard' && task.category !== 'done').length,
  ...
  return (
    <div className={`min-h-screen transition-all duration-300 ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} p-4 md:p-8`}>
      <style>{`
      ...
      <div className="max-w-[1600px] mx-auto font-body">
        <header className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-8">
          <div className="flex items-center gap-4 relative">
            <button
              ...
              className="bg-indigo-600 p-3 rounded-2xl shadow-xl shadow-indigo-500/30 text-white transition-transform active:scale-95"
              aria-label="Changer le logo"
            >
              <CurrentLogoIcon size={24} />
            </button>

            {showLogoPicker && (
              <div className={`absolute top-16 left-0 p-2 rounded-2xl shadow-2xl flex gap-1 z-[60] ${darkMode ? 'bg-slate-900 border border-slate-800' : 'bg-white border border-slate-200'}`}>
            ...
            <div>
              <h1 className="text-3xl md:text-4xl font-brand uppercase tracking-tighter italic leading-none">
                Pense-Bête <span className="text-indigo-600">Ultimate</span>
              </h1>
              <div className="flex flex-col gap-2 mt-2">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <div className={`w-40 h-2 rounded-full overflow-hidden ${darkMode ? 'bg-slate-800' : 'bg-slate-200'}`}>
                    <div className={`h-full ${completionRate === 100 ? 'bg-emerald-500' : 'bg-indigo-600'}`} style={{ width: `${completionRate}%` }} />
                  </div>
          ...
          <div className="flex flex-wrap gap-3 w-full xl:w-auto">
            <div className={`flex items-center px-4 py-3 rounded-2xl border min-w-[240px] ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              <Search size={16} className="text-slate-400 mr-2" />
              <input
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={activeView === 'timesheet' ? 'Chercher un projet, une ligne de temps...' : 'Chercher une tâche, un tag, une note...'}
                className="bg-transparent outline-none text-sm w-full"
              />
            </div>

            <div className={`flex items-center gap-2 px-3 py-2 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
              {user ? (
                <>
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-xl bg-indigo-600 text-white text-xs font-brand uppercase tracking-wider disabled:opacity-50 flex items-center gap-2"
                >
        ...
        {!firebaseReady && (
          <div className={`mb-8 p-4 rounded-2xl border ${darkMode ? 'bg-amber-950/30 border-amber-900 text-amber-200' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>
            <p className="text-sm font-semibold">Connexion Google non configurée.</p>
            <p className="text-xs mt-1">
              Ajoute les variables Firebase dans Vercel puis redeploie pour activer le partage et la sauvegarde cloud.
            </p>
          </div>
        )}

        {saveError && (
          <div className={`mb-8 p-4 rounded-2xl border ${darkMode ? 'bg-red-950/30 border-red-900 text-red-200' : 'bg-red-50 border-red-200 text-red-700'}`}>
            <p className="text-sm font-semibold">{saveError}</p>
          </div>
        )}

        {authLoading || !dataHydrated ? (
          <div className={`mb-8 p-8 rounded-[2rem] border flex items-center justify-center ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            <div className="flex items-center gap-3 text-sm text-slate-400">
              <Cloud size={18} className="animate-pulse" /> Chargement des données...
            </div>
          </div>
        ) : null}

        <div className={`mb-8 inline-flex p-1 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
          <button
        ...
        {showTagEditor && (
          <section className={`mb-8 p-6 rounded-[2rem] border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-brand text-xs uppercase tracking-widest flex items-center gap-2">
                <Palette size={16} /> Tags & filtres
              </h3>
              <button onClick={() => setShowTagEditor(false)} className="text-slate-400 hover:text-red-500" aria-label="Fermer">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex flex-wrap gap-3 mb-5">
              {tags.map((tag) => (
                ...
                >
                  <button type="button" onClick={() => setActiveTagFilter((prev) => (prev === tag.name ? null : tag.name))} className="cursor-pointer">
                    {tag.name}
                  </button>
                  {tag.name !== 'Général' && (
                    <button type="button" onClick={() => removeTag(tag.id, tag.name)} className="hover:text-black/50" aria-label={`Supprimer ${tag.name}`}>
                      <X size={13} />
                    </button>
                  )}
                </div>
              ...
              {activeTagFilter && (
                <button onClick={() => setActiveTagFilter(null)} className="text-xs text-indigo-500 font-bold uppercase">
                  Effacer le filtre
                </button>
              )}
            </div>
            ...
            <div className="flex gap-2 max-w-md">
              ...
              />
              <button onClick={addTag} className="bg-indigo-600 text-white px-4 py-3 rounded-xl font-bold text-xs uppercase">
                Ajouter
              </button>
            </div>
          </section>
        ...
        {showStats && activeView === 'board' && (
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <div key={label} className={`p-5 rounded-3xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <p className="text-[10px] font-brand uppercase text-slate-400 tracking-widest mb-1">{label}</p>
        ...
        {showStats && activeView === 'timesheet' && (
          <section className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ['Heures', formatHours(timesheetStats.totalHours)],
              ['Jours', timesheetStats.uniqueDays],
              ['Moy./jour', formatHours(timesheetStats.avgPerDay)],
              ['Top projet', timesheetStats.topProject],
            ].map(([label, value]) => (
              <div key={label} className={`p-5 rounded-3xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <p className="text-[10px] font-brand uppercase text-slate-400 tracking-widest mb-1">{label}</p>
                <p className="text-3xl font-brand text-indigo-600">{value}</p>
        ...
        {showArchive && activeView === 'board' && (
          <section className={`mb-8 p-6 rounded-[2rem] border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
            <div className="flex justify-between items-center mb-5">
              <h3 className="font-brand text-xs uppercase tracking-widest flex items-center gap-2">
                <Archive size={16} /> Archives
              </h3>
              <span className="text-xs font-bold text-slate-400">{filteredArchivedTasks.length} tâche(s)</span>
            </div>
            ...
            {filteredArchivedTasks.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {filteredArchivedTasks.map((task) => {
                  ...
                  return (
                    <div key={task.id} className={`p-5 rounded-[2rem] border ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <span className={`text-[9px] font-brand font-black uppercase px-2.5 py-1 rounded-xl text-white ${tagStyle?.color || 'bg-slate-500'}`}>
                          {task.tag}
                        </span>
                        <button onClick={() => deleteArchivedTask(task.id)} className="text-slate-400 hover:text-red-500" aria-label="Supprimer l'archive">
                          <Trash2 size={14} />
                        </button>
                      </div>
                      ...
                      <TaskSubtasks task={task} darkMode={darkMode} onToggle={toggleSubtask} onAddSubtask={addSubtaskToTask} archived />

                      <div className="flex justify-between items-center pt-4 border-t dark:border-slate-800">
                        <span className="text-[10px] text-slate-400 uppercase font-bold">{task.dueDate || 'Sans date'}</span>
                        <button onClick={() => restoreTask(task.id)} className="text-indigo-600 px-3 py-2 rounded-2xl text-[10px] font-brand font-black uppercase hover:bg-indigo-50 dark:hover:bg-slate-800">
                          Restaurer
                        </button>
                      </div>
                    </div>
        ...
        {activeView === 'board' ? (
            ...
            <section className={`max-w-5xl mx-auto mb-10 p-8 rounded-[2.5rem] border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                <div className="md:col-span-6">
                  <label className="text-[10px] font-brand uppercase text-indigo-500 mb-2 block ml-2">Description</label>
                  <input
                    type="text"
                    placeholder="Quelle est la mission ?"
                    value={form.text}
                    onChange={(e) => setForm((prev) => ({ ...prev, text: e.target.value }))}
                    className={`w-full px-5 py-4 rounded-2xl outline-none font-bold text-lg ${darkMode ? 'bg-slate-800 text-white placeholder:text-slate-600' : 'bg-slate-50 text-slate-700 placeholder:text-slate-300'}`}
                  />
                </div>

                <div className="md:col-span-3">
                  <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Échéance</label>
                  <div className={`flex items-center px-4 py-4 rounded-2xl ${darkMode ? 'bg-slate-800' : 'bg-slate-50'}`}>
                ...
                <div className="md:col-span-3">
                  <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Catégorie</label>
                  <select
                    value={form.tag}
                    onChange={(e) => setForm((prev) => ({ ...prev, tag: e.target.value }))}
                    className={`w-full px-4 py-4 rounded-2xl border-none font-bold text-xs uppercase outline-none appearance-none cursor-pointer ${darkMode ? 'bg-slate-800' : 'bg-slate-50'}`}
                  >
                    {tags.map((tag) => (
                ...
                <div className="md:col-span-12">
                  <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Note</label>
                  <textarea
                    placeholder="Ajouter un détail, un rappel ou un contexte..."
                    value={form.note}
                    onChange={(e) => setForm((prev) => ({ ...prev, note: e.target.value }))}
                    rows={3}
                    className={`w-full px-5 py-4 rounded-2xl outline-none text-sm resize-none ${darkMode ? 'bg-slate-800 text-white placeholder:text-slate-600' : 'bg-slate-50 text-slate-700 placeholder:text-slate-300'}`}
                  />
                </div>
              </div>

              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-8 pt-6 border-t dark:border-slate-800">
                <div>
                  <label className="text-[9px] font-brand uppercase text-slate-400 mb-1 block">Priorité</label>
                  <select
                    value={form.priority}
                ...
                <button onClick={addTask} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-brand text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20">
                  Lancer la mission
                </button>
              </div>
            </section>
            ...
            <main className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
              {COLUMNS.map((col, columnIndex) => {
                ...
                return (
                  <section
                    key={col.id}
                    className={`flex flex-col min-h-[560px] rounded-[2.5rem] overflow-hidden border-2 ${darkMode ? 'border-slate-800 bg-slate-900/40' : `border-slate-100 ${col.light}`}`}
                  >
                    <div className={`${col.color} p-5 text-white relative overflow-hidden`}>
                      ...
                      <div className="flex items-center justify-between gap-3 relative z-10">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-brand font-black uppercase tracking-[0.12em] border shadow-sm ${col.badge}`}>
                      ...
                      {(() => {
                        const avgProgress = colTasks.length
                          ? Math.round(
                              colTasks.reduce((acc, t) => acc + getSubtasksProgress(t.subtasks || []), 0) /
                                colTasks.length
                            )
                          : 0;

                        const urgentCount = colTasks.filter((t) => getTaskStatus(t.dueDate) === 'urgent').length;
                        const lateCount = colTasks.filter((t) => getTaskStatus(t.dueDate) === 'retard').length;
                          ...
                          <div className="mt-3 space-y-2.5">
                            ...
                            <div>
                              ...
                              <div className="mt-1 flex items-center justify-between text-[8px] font-brand uppercase tracking-[0.12em] opacity-80">
                                ...
                                <span>{lateCount} retard · {urgentCount} urgent</span>
                              </div>
                            </div>
                          </div>
                        );
                      ...
                      {colTasks.length === 0 ? (
                        ...
                        colTasks.map((task) => {
                          ...
                          return (
                            <article
                              key={task.id}
                              className={`p-5 rounded-[2rem] border-2 shadow-sm transition-all hover:shadow-lg hover:-translate-y-0.5 ${priorityStyle.card} ${status === 'retard' && task.category !== 'done' ? 'ring-2 ring-red-500/20' : ''}`}
                            >
                              <div className="flex items-start justify-between gap-3 mb-3">
                                <div className="flex flex-wrap gap-2">
                                  <span className={`text-[9px] font-brand font-black uppercase px-2.5 py-1 rounded-xl text-white ${taskTag?.color || 'bg-slate-500'}`}>
                                    {task.tag}
                                  </span>
                                  <span className={`text-[9px] font-brand font-black uppercase px-2.5 py-1 rounded-xl ${priorityStyle.badge}`}>
                                    {priorityStyle.label}
                                  </span>
                                </div>
                                <button onClick={() => deleteTask(task.id)} className="text-slate-400 hover:text-red-500" aria-label="Supprimer la tâche">
                                  <Trash2 size={15} />
                                </button>
                              </div>

                              <h3 className={`text-base font-bold leading-tight mb-2 ${task.category === 'done' ? 'line-through text-slate-400' : darkMode ? 'text-white' : 'text-slate-800'}`}>
                                {task.text}
                              </h3>

                              {task.note && <p className="text-xs text-slate-400 italic mb-4">"{task.note}"</p>}

                              <TaskSubtasks task={task} darkMode={darkMode} onToggle={toggleSubtask} onAddSubtask={addSubtaskToTask} />

                              {task.dueDate && (
                                <div className="flex flex-wrap gap-2 mb-4">
                                  <span className={`text-[10px] font-brand font-black uppercase px-2.5 py-1 rounded-xl flex items-center gap-1 ${darkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>
                                    <Clock size={10} /> {task.dueDate}
                                  </span>
                                  {dueMeta && (
                                    <span title={dueMeta.detailLabel} className={`text-[10px] font-brand font-black uppercase px-2.5 py-1 rounded-xl ${getDeadlineBadgeClass(status, darkMode)}`}>
                                      {dueMeta.shortLabel}
                                    </span>
                                  )}
                                </div>
                              ...
                              <div className="mb-4">
                                <div className="flex items-center justify-between mb-1.5">
                                  <span className="text-[9px] font-brand uppercase text-slate-400">Niveau</span>
                                  <span className="text-[9px] font-brand uppercase text-slate-500">{task.priority}/3</span>
                                </div>
                                <div className={`h-2 rounded-full overflow-hidden ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
                                  <div className={`h-full rounded-full ${priorityStyle.bar}`} style={{ width: `${(task.priority / 3) * 100}%` }} />
                                </div>
                              </div>
          ...
          <section className="space-y-8">
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
              <div className={`xl:col-span-1 p-6 rounded-[2rem] border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <div className="flex items-center gap-2 mb-5">
                  <Receipt size={18} className="text-indigo-500" />
                  <h3 className="font-brand text-xs uppercase tracking-widest">Nouvelle ligne timesheet</h3>
                </div>

                <div className="space-y-4">
                  <div>
                    <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Date</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Tâche liée</label>
                    <select
                  ...
                  <div>
                    <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Projet / tag</label>
                    <select
                  ...
                  <div>
                    <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Description</label>
                    <textarea
                      rows={4}
                  ...
                  <div>
                    <label className="text-[10px] font-brand uppercase text-slate-400 mb-2 block ml-2">Heures</label>
                    <input
                      type="number"
                      step="0.5"
                      min="0"
              ...
              <div className={`xl:col-span-2 p-6 rounded-[2rem] border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <div className="flex items-center justify-between gap-3 mb-5">
                  <div>
                    <h3 className="font-brand text-xs uppercase tracking-widest flex items-center gap-2">
                      <Receipt size={16} className="text-indigo-500" /> Timesheet
                    </h3>
                    <p className="text-xs text-slate-400 mt-1">Suivi du temps par tâche, projet et journée.</p>
                  </div>
                  <div className={`px-3 py-2 rounded-2xl text-xs font-brand uppercase ${darkMode ? 'bg-slate-800 text-slate-300' : 'bg-slate-50 text-slate-600'}`}>
                ...
                <div className="space-y-4">
                  {filteredTimesheetEntries.length === 0 ? (
                    <div className="py-16 text-center text-sm italic text-slate-400">Aucune ligne timesheet.</div>
                  ) : (
                    filteredTimesheetEntries.map((entry) => {
                      const linkedTask = tasks.find((task) => task.id === entry.taskId) || archivedTasks.find((task) => task.id === entry.taskId);
                        ...
                        >
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              <span className={`text-[9px] font-brand font-black uppercase px-2.5 py-1 rounded-xl ${projectTag ? `${projectTag.color} text-white` : darkMode ? 'bg-slate-800 text-slate-200' : 'bg-slate-200 text-slate-700'}`}>
                                {entry.project}
                              </span>
                              <span className={`text-[9px] font-brand font-black uppercase px-2.5 py-1 rounded-xl ${darkMode ? 'bg-slate-800 text-slate-300' : 'bg-white text-slate-500'}`}>
                                {entry.date}
                              </span>
